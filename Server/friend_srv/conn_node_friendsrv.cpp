...
void conn_node_friendsrv::handle_friend_info_request()
{
	...
	do
	{
		...
		if (get_all_friend_redis_player(player, redis_players) != 0)
		{
			ret = ERROR_ID_SERVER;
...
void conn_node_friendsrv::handle_friend_create_group_request()
{
	...
	do
	{
		...
		if (members.size() > 0)
		{
			get_more_redis_player(playerIds, redis_players);
			notify_friend_list_change(player, change_info);
		}
		save_friend_player(player);
	} while(0);
...
void conn_node_friendsrv::handle_friend_recommend_request()
{
	...
	get_more_redis_player(recommendIds, redis_players);
	resp.n_players = 0;
...
void conn_node_friendsrv::handle_friend_gift_cost_answer()
{
	...
	do
	{
		...
		do
		{
			...
			PlayerRedisInfo *redis_target = get_redis_player(target->player_id);
			if (redis_target)
			{
...
void conn_node_friendsrv::handle_friend_chat_request()
{
	...
	do
	{
		...
		PlayerRedisInfo *redis_receiver = get_redis_player(req->recvplayerid);
		if (!redis_receiver)
		{
			ret = ERROR_ID_FRIEND_CHAT_RECEIVER;
...
void conn_node_friendsrv::handle_player_online_notify()
{
	...
	do
	{
		...
		if (watchers)
		{
			get_more_redis_player(*watchers, redis_players);
...
void conn_node_friendsrv::handle_friend_rename_request() //玩家改名
{
	...
	do
	{
		...
		if (get_more_redis_player(*notice_player, redis_players) != 0)
		{
			break;
		}
