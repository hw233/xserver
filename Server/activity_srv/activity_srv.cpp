...
extern struct event sg_second_timer_event;
extern struct timeval sg_second_timer_val;
extern void cb_second_timeout(evutil_socket_t, short, void* /*arg*/);
...
//			break;
    ...
    LOG_INFO("activity_srv run %lu", pid);
	...
	ret = game_add_connect_event((struct sockaddr *)&sin, sizeof(sin), &conn_node_activitysrv::connecter);
	if (ret <= 0)
		goto done;
	...
	read_all_activity_excel_data();
	init_redis_keys(sg_server_id);
	load_activity_db();

	sg_second_timer_val.tv_sec = 1;
	sg_second_timer_event.ev_callback = cb_second_timeout;
	...
	LOG_INFO("activity_srv stoped[%d]", ret);
	return (ret);
}
