...
listen_node_item::listen_node_item()
{
}
...
int listen_node_item::listen_pre_func()
{
	return (0);	
}

int listen_node_item::listen_after_func(evutil_socket_t fd)
{
	return (0);		
}
...
conn_node_base * listen_node_item::get_conn_node(evutil_socket_t fd)
{

	conn_node_item *ret = new conn_node_item();
	if (!ret) {
...
//	event_set(&ret->ev_write, fd, EV_WRITE, on_write, ret);
	...
	conn_node_item::add_map_fd_nodes(ret);
	conn_node_item::add_map_player_id_nodes(ret);
	conn_node_item::add_map_open_id_nodes(ret);
