...
conn_node_rank *conn_node_rank::server_node;

conn_node_rank::conn_node_rank()
{
...
int conn_node_rank::recv_func(evutil_socket_t fd)
{
	for (;;) {
		int ret = get_one_buf();
		if (ret == 0) {
...
int conn_node_rank::transfer_to_gamesrv()
{
	int ret = 0;
	PROTO_HEAD *head;
...
int conn_node_rank::transfer_to_mailsrv()
{
	int ret = 0;
	PROTO_HEAD *head;
	head = (PROTO_HEAD *)buf_head();
...
int conn_node_rank::broadcast_to_client()
{
