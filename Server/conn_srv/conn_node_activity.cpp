...
conn_node_activity *conn_node_activity::server_node;

conn_node_activity::conn_node_activity()
{
...
int conn_node_activity::recv_func(evutil_socket_t fd)
{
	for (;;) {
		int ret = get_one_buf();
		if (ret == 0) {
...
int conn_node_activity::dispatch_message()
{
	...
	switch (cmd)
	{
		...
		case SERVER_PROTO_ACTIVITYSRV_REWARD_REQUEST:
		...
		case SERVER_PROTO_ACTIVITY_SHIDAMENZONG_GIVE_REWARD_REQUEST:
...
int conn_node_activity::transfer_to_client()
{
	uint32_t old_len;
	PROTO_HEAD *head;
...
int conn_node_activity::transfer_to_gamesrv()
{
	int ret = 0;
	PROTO_HEAD *head;
...
int conn_node_activity::transfer_to_mailsrv()
{
	int ret = 0;
	PROTO_HEAD *head;
	head = (PROTO_HEAD *)buf_head();
...
int conn_node_activity::transfer_to_ranksrv()
{
	int ret = 0;
	PROTO_HEAD *head;
	head = (PROTO_HEAD *)buf_head();
...
int conn_node_activity::transfer_to_guildsrv()
{
	int ret = 0;
	PROTO_HEAD *head;
	head = (PROTO_HEAD *)buf_head();
...
int conn_node_activity::broadcast_to_client()
{
...
int conn_node_activity::broadcast_to_all_client()
{
	conn_node_client *client;
	PROTO_HEAD *real_head;
	PROTO_HEAD_CONN_BROADCAST *head = (PROTO_HEAD_CONN_BROADCAST *)buf_head();
