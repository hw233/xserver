...
conn_node_mail *conn_node_mail::server_node;

conn_node_mail::conn_node_mail()
{
...
int conn_node_mail::recv_func(evutil_socket_t fd)
{
	for (;;) {
		int ret = get_one_buf();
		if (ret == 0) {
...
int conn_node_mail::transfer_to_client()
{
	uint32_t old_len;
	PROTO_HEAD *head;
	...
	switch (cmd)
	{
		case SERVER_PROTO_MAIL_GIVE_ATTACH_REQUEST:
...
int conn_node_mail::transfer_to_gamesrv()
{
	int ret = 0;
	PROTO_HEAD *head;
