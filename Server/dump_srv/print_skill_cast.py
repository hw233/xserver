...
#cast skill
    if msg_id == 10200:
        ...
        print oldtime.time(), ": %lu cast skill %u from pos %.1f %.1f" % (player_id, req.skillid, req.cur_pos.pos_x, req.cur_pos.pos_z)

    if msg_id == 10202:
        req = cast_skill_pb2.skill_cast_notify()
        req.ParseFromString(pb_data)
        oldtime=datetime.datetime.now()
        print oldtime.time(), ": %lu notify skill %u from pos %.1f %.1f" % (req.playerid, req.skillid, req.cur_pos.pos_x, req.cur_pos.pos_z)
