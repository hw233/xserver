...
encode_integer(uint32_t v, uint8_t * data, int size) {
	if ((size_t)size < SIZEOF_LENGTH + sizeof(v))
		return -1;
	data[4] = v & 0xff;
	data[5] = (v >> 8) & 0xff;
	data[6] = (v >> 16) & 0xff;
...
static int decode_array_object_c(struct field *f, uint8_t * stream, int sz, void *ret, struct pool *pool)
{
	...
	*(uint32_t *)ret = n_size;
	ret = (char *)ret + sizeof(uint32_t);
	void ***p = (void ***)ret;
	(*p) = (void **)pool_alloc(pool, sizeof(void *) * n_size);
	if (!*p)
		return -1;
...
//		cb(args);
	...
	case SPROTO_TINTEGER: {
		...
		if (len == sizeof(uint32_t)) {
			...
			*p = (uint64_t *)pool_alloc(pool, sizeof(uint64_t) * sz / sizeof(uint32_t));
			if (!*p)
				return -1;

			*n_size = sz/sizeof(uint32_t);
			for (i=0;(uint32_t)i<sz/sizeof(uint32_t);i++) {
...
//				cb(args);
			...
			if (sz % sizeof(uint64_t) != 0)
				return -10;
			uint64_t **p = (uint64_t **)ret;
			*p = (uint64_t *)pool_alloc(pool, sizeof(uint64_t) * sz / sizeof(uint64_t));
			if (!*p)
				return -1;

			*n_size = sz/sizeof(uint64_t);			
			for (i=0;(uint32_t)i<sz/sizeof(uint64_t);i++) {
...
//				cb(args);
	...
	{
		bool **p = (bool **)ret;
		*p = (bool *)pool_alloc(pool, sizeof(bool) * sz);
		if (!*p)
			return -30;
		*n_size = sz;
		for (i=0;(uint32_t)i<sz;i++) {
...
//			cb(args);
	...
	case SPROTO_TSTRING:
	case SPROTO_TSTRUCT:
		return decode_array_object_c(f, stream, sz, (char *)ret - sizeof(uint32_t), pool);
	default:
		return -40;
	}
//	(*ret) += sizeof(void *);
...
//						cb(&args);
				...
				case SPROTO_TSTRUCT: {
					uint32_t sz = todword(currentdata);
					int struct_ret = sproto_decode_c(f->st, currentdata+SIZEOF_LENGTH, sz, (void **)p, pool);
					if (struct_ret < 0)
						return -60;
					(p) = (char *)p + sizeof(void *);
