...
//			buff_manager::create_buff(114400018, player, player, true);
	...
	{ //如果是断线重连，不再通知玩家数据
		...
		player_ready_enter_scene(player, extern_data, true);
			...
			//获取玩家离线缓存
		fast_send_msg_base(&conn_node_gamesrv::connecter, extern_data, SERVER_PROTO_GET_OFFLINE_CACHE_REQUEST, 0, get_seq());
	}
	else
	{
		AttrMap attrs;
...
//	get_scene_birth_pos(player->data->scene_id, NULL, NULL, NULL, &direct);
	...
	/// 用户上线通知的排行榜服务器
	fast_send_msg_base(&conn_node_gamesrv::connecter, extern_data, SERVER_PROTO_PLAYER_ONLINE_NOTIFY, 0, 0);
...
//	player->tidy_bag();
	...
	for (uint32_t i = 0; i < player->data->bag_grid_num; ++i)
	{
		...
		if (item_is_baguapai(player->data->bag[i].id))
		{
			...
			bagua_data[resp.n_grids].star = player->data->bag[i].baguapai.star;
			bagua_data[resp.n_grids].main_attr_val = player->data->bag[i].baguapai.main_attr_val;
			uint32_t attr_num = 0;
			for (int j = 0; j < MAX_BAGUAPAI_MINOR_ATTR_NUM; ++j)
			{
				if (player->data->bag[i].baguapai.minor_attrs[j].id == 0)
				{
					break;
				}
				...
				bagua_attr[resp.n_grids][attr_num].id = player->data->bag[i].baguapai.minor_attrs[j].id;
				bagua_attr[resp.n_grids][attr_num].val = player->data->bag[i].baguapai.minor_attrs[j].val;
				attr_num++;
			}
...
static int handle_bag_use_request(player_struct *player, EXTERN_DATA *extern_data)
{
	...
	int ret = 0;
	uint32_t item_id = 0;
	std::map<uint32_t, uint32_t> add_item_list;
	do
	{
		...
		if (prop_config->ItemEffect == IUE_FIND_TARGET)
		{
			ret = ChengJieTaskManage::CanUseFollowItem(player);
			if (ret != 190500154)
			{
				break;
			}
		}
		else if (prop_config->ItemEffect == IUE_TRANSFER_SCENE)
		{
			if (!player->scene || !player->scene->can_transfer(2))
			{
				ret = ERROR_ID_CAN_NOT_TRANSFER_IN_RAID;
				break;
			}
			if (player->is_in_raid())
			{
		...
		else if (prop_config->ItemEffect == IUE_EXTEND_FRIEND_NUM)
		{
			//换一个消息号，转发到好友服处理
		...
		if (prop_config->CostTime > 0)
		{
			...
			{
				player->begin_sing(SING_TYPE__XUNBAO, prop_config->CostTime, pos, false, true);
			}
			else
			{
				player->begin_sing(SING_TYPE__USE_PROP, prop_config->CostTime, pos, true, true);
			}
		}
		else
		{
			ret = player->use_prop(pos, use_all, &add_item_list);
		}
	} while(0);
	...
	for (std::map<uint32_t, uint32_t>::iterator iter = add_item_list.begin(); iter != add_item_list.end() && resp.n_itemlist < 50; ++iter)
	{
...
static int handle_sing_begin_request(player_struct *player, EXTERN_DATA *extern_data)
{
	...
	do
	{
		...
		player->begin_sing(sing_type, sing_time, 0, broadcast, include_myself);
	} while(0);
...
static int  handle_puton_fashion_request(player_struct *player, EXTERN_DATA *extern_data)
{
	...
	player->notify_attr(attrs, true, true);
	player->add_task_progress(TCT_WEAR_FASHION, fashionId, 1);
done:
...
static int handle_add_horse_soul_level_request(player_struct *player, EXTERN_DATA *extern_data)
{
	...
	std::map<uint64_t, struct CastSpiritTable*>::iterator it = horse_soul_config.find(player->data->horse_attr.soul); //坐骑铸灵配置
...
static int handle_shop_buy_request(player_struct *player, EXTERN_DATA *extern_data)
{
	...
	do
	{
		...
		uint32_t need_money = goods_config->Price * buy_num;
...
static int handle_baguapai_wear_request(player_struct *player, EXTERN_DATA *extern_data)
{
	...
	do
	{
		...
		for (std::map<uint32_t, bag_grid_data *>::iterator iter = grid_datas.begin(); iter != grid_datas.end(); ++iter)
		{
			...
			card_info->star = grid->baguapai.star;
			card_info->main_attr_val = grid->baguapai.main_attr_val;
			memcpy(card_info->minor_attrs, grid->baguapai.minor_attrs, sizeof(card_info->minor_attrs));
			player->add_task_progress(TCT_WEAR_BAGUA, card_id, 1);
		}
...
static int handle_baguapai_decompose_request(player_struct *player, EXTERN_DATA *extern_data)
{
	...
	do
	{
		for (std::set<uint32_t>::iterator iter = grid_ids.begin(); iter != grid_ids.end(); ++iter)
		{
			...
			if (grid->baguapai.star > 0)
			{
				BaguaStarTable *star_config = get_bagua_star_config(grid->baguapai.star);
				if (!star_config)
				{
					ret = ERROR_ID_NO_CONFIG;
					LOG_ERR("[%s:%d] player[%lu] get star config failed, pos:%u, item_id:%u, star:%u", __FUNCTION__, __LINE__, extern_data->player_id, pos, grid->id, grid->baguapai.star);
					break;
				}
...
int CheckCanAcceptChengjieTask(player_struct *player, STChengJie * task)
{
	...
	if (player->get_uuid() == task->investor || player->get_uuid() == task->pid)
	{
		return 190500135;
	}
	if (task->complete)
...
static int handle_sync_guild_skill_request(player_struct *player, EXTERN_DATA *extern_data)
{
	...
	memset(player->data->guild_skill_attr, 0, sizeof(double) * PLAYER_ATTR_FIGHT_MAX);
...
static int handle_sync_guild_info_request(player_struct *player, EXTERN_DATA *extern_data)
{
	...
	{
		player->add_task_progress(TCT_JOIN_GUILD, (player->data->guild_office == 1 ? 2 : 1), 1);
	}
...
static int on_login_send_live_skill(player_struct *player, EXTERN_DATA *extern_data)
{
	if (player->data->live_skill.level[LIVE__SKILL__TYPE__MEDICINE] == 0)
	{
		...
		player->data->live_skill.book[LIVE__SKILL__TYPE__COOK] = 20; //todo 读配置
		player->data->live_skill.book[LIVE__SKILL__TYPE__MEDICINE] = 20;
...
static int handle_xunbao_pos_request(player_struct *player, EXTERN_DATA *extern_data)
{
	...
	if (empty_num < 3)
	{
		...
		player->send_system_notice(190500284, args);
	}
