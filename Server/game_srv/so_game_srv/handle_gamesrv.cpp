...
static int handle_learn_fuwen_request(player_struct *player, EXTERN_DATA *extern_data)
{
	...
	for (; toNum < num; ++toNum)
	{
		...
		else
		{
			...
			if (iter != skill_lv_config.end())
			{
				...
				pFuwen->lv += 1;
				notify.lv = pFuwen->lv;
				player->add_achievement_progress(ACType_SKILL_FUWEN_LEVEL_UP, 0, 0, 1);
			}
	...
	player->add_achievement_progress(ACType_SKILL_FUWEN_LEVEL_NUM, 0, 0, 1);
	return (0);
}
...
static int handle_set_fuwen_request(player_struct *player, EXTERN_DATA *extern_data)
{
	...
	if (send.ret == 0)
	{
		player->add_achievement_progress(ACType_SKILL_FUWEN_WEAR, 0, 0, player->m_skill.GetFuwenWearNum());
	}
	...
	if (send.ret == 0)
	{
		if (num == 0)
		{
			...
			while (player->data->live_skill.exp[type] >= table->Exp && table != NULL)
			{
				...
				player->add_achievement_progress(ACType_LIVE_SKILL_LEVEL, type, player->data->live_skill.level[type], 1);
			}
...
static int handle_produce_medicine_request(player_struct *player, EXTERN_DATA *extern_data)
{
	...
	player->add_achievement_progress(ACType_LIVE_SKILL_ENERGY, type, 0, table->NeedJingli);
...
static int handle_skill_cast_request(player_struct *player, EXTERN_DATA *extern_data)
{
	...
	if (!player->is_in_buff3())
	{
		...
		active_config = get_config_by_id(config->SkillAffectId, &active_skill_config);
		if (!active_config)
		{
			LOG_ERR("%s: %lu skill %u not active_config", __FUNCTION__, extern_data->player_id, req->skillid);
			skill_cast_request__free_unpacked(req, NULL);
...
//			LOG_ERR("[%s:%d] player[%lu] icon is using, icon:%u", __FUNCTION__, __LINE__, extern_data->player_id, replace_icon);
		...
		player->add_achievement_progress(ACType_HEAD_REPLACE, 0, 0, 1);
	} while(0);
		...
		if (config->TaskType == TT_TRUNK)
		{
			if (config->FollowTask > 0)
			{
				...
				if (config->ChapterId != get_task_chapter_id(config->FollowTask))
				{
					...
					player->add_achievement_progress(ACType_TASK_CHAPTER, config->ChapterId, 0, 1);
				}
			}
		}
...
int handle_chat_broadcast_request(player_struct *player, EXTERN_DATA *extern_data)
{
	...
	player->add_achievement_progress(ACType_CHAT_BOARDCAST, 0, 0, 1);
...
static int notify_equip_list(player_struct *player, EXTERN_DATA *extern_data)
{
	...
	EquipEnchantAttrData cur_attr[MAX_EQUIP_NUM][MAX_EQUIP_ENCHANT_NUM];
	EquipEnchantAttrData* cur_attr_point[MAX_EQUIP_NUM][MAX_EQUIP_ENCHANT_NUM];
	EquipEnchantAttrData rand_attr[MAX_EQUIP_NUM][MAX_EQUIP_ENCHANT_NUM][MAX_EQUIP_ENCHANT_RAND_NUM];
	EquipEnchantAttrData* rand_attr_point[MAX_EQUIP_NUM][MAX_EQUIP_ENCHANT_NUM][MAX_EQUIP_ENCHANT_RAND_NUM];
	...
	for (int k = 0; k < MAX_EQUIP_NUM; ++k)
	{
		...
		for (int i = 0; i < MAX_EQUIP_ENCHANT_NUM; ++i)
		{
			...
			equip_enchant_attr_data__init(&cur_attr[equip_num][enchant_num]);
			...
			if (enchant_info.rand_attr[0].id > 0)
			{
				for (int j = 0; j < MAX_EQUIP_ENCHANT_RAND_NUM; ++j)
				{
					...
					equip_enchant_attr_data__init(&rand_attr[equip_num][enchant_num][rand_num]);
...
static int handle_equip_stair_up_request(player_struct *player, EXTERN_DATA *extern_data)
{
	...
	do
	{
		...
		player->add_achievement_progress(ACType_EQUIP_STAIR, player->data->equip_list[0].stair, 0, 1);
	} while(0);
...
static int handle_equip_enchant_request(player_struct *player, EXTERN_DATA *extern_data)
{
	...
	do
	{
		...
		for (; rand_idx < MAX_EQUIP_ENCHANT_RAND_NUM; ++rand_idx)
		{
			...
			EquipAttribute *attr_config = get_equip_enchant_attr_config(star_config->DatabaseSelection, attr_type);
			if (!attr_config)
			{
				ret = ERROR_ID_NO_CONFIG;
			...
			enchant_info->rand_attr[rand_idx].pool = attr_config->Database;
		...
		player->add_achievement_progress(ACType_EQUIP_ENCHANT, 0, 0, 1);
	} while(0);

	EquipEnchantAnswer resp;
	equip_enchant_answer__init(&resp);

	EquipEnchantAttrData rand_data[MAX_EQUIP_ENCHANT_RAND_NUM];
	EquipEnchantAttrData* rand_data_point[MAX_EQUIP_ENCHANT_RAND_NUM];
	...
	if (ret == 0 && equip_info != NULL)
	{
		for (int i = 0; i < MAX_EQUIP_ENCHANT_RAND_NUM; ++i)
		{
			...
			equip_enchant_attr_data__init(&rand_data[i]);
...
static int handle_equip_enchant_retain_request(player_struct *player, EXTERN_DATA *extern_data)
{
	...
	int ret = 0;
	EquipInfo *equip_info = NULL;
	EquipEnchantAttrData retain_attr;
	equip_enchant_attr_data__init(&retain_attr);
	do
	{
		equip_info = player->get_equip(type);
		if (!equip_info || equip_info->stair == 0)
		...
		if (!(retain_index == 0 && enchant_info->cur_attr.id > 0))
		{
			...
			player->calculate_attribute(true);
			player->add_achievement_progress(ACType_EQUIP_ENCHANT_COLOR_NUM, 0, 0, 0);
		}
...
//		}
		...
		if (equip_info->inlay[index] > 0)
		{
			...
			equip_info->inlay[index] = 0;
			player->add_achievement_progress(ACType_EQUIP_STRIP, 0, 0, 1);
		}
		...
		player->add_achievement_progress(ACType_EQUIP_INLAY_QULITY_NUM, 0, 0, 1);
	} while(0);
...
static int handle_equip_gem_compose_request(player_struct *player, EXTERN_DATA *extern_data)
{
	...
	do
	{
		...
		player->add_achievement_progress(ACType_EQUIP_GEM_COMPOSE, gem_config->Level, 0, 1);
	} while(0);
...
static int handle_set_fashion_color_request(player_struct *player, EXTERN_DATA *extern_data)
{
	...
	player->notify_attr(attrs, true, true);
	player->add_achievement_progress(ACType_FASHION_COLOR, 0, 0, 1);
done:
	send.id = fashoinid;
	send.color = color;
	send.isdown = isdown;
...
static int handle_set_cur_horse_request(player_struct *player, EXTERN_DATA *extern_data)
{
	...
	player->add_achievement_progress(ACType_HORSE_SET_CUR, 0, 0, 1);
done:
...
static int handle_add_horse_exp_request(player_struct *player, EXTERN_DATA *extern_data) //修灵
{
	...
	player->add_achievement_progress(ACType_HORSE_ADD_EXP, 0, 0, add);
...
static int handle_add_horse_step_request(player_struct *player, EXTERN_DATA *extern_data)
{
	...
	player->add_achievement_progress(ACType_HORSE_STEP, player->data->horse_attr.step, 0, 1);
done:
...
static int handle_add_horse_soul_request(player_struct *player, EXTERN_DATA *extern_data)
{
	...
	player->add_achievement_progress(ACType_HORSE_ADD_SOUL, 0, 0, 1);
done:
	send.soul = cur;
	player->calc_horse_attr();
	send.power = player->data->horse_attr.power;
...
static int handle_shop_buy_request(player_struct *player, EXTERN_DATA *extern_data)
{
	...
	do
	{
		...
		switch(money_type)
		{
			...
			// 	break;
			// case 8: //赏金货币
			// 	has_money = (uint32_t)player->get_attr(PLAYER_ATTR_SHANGJIN_COIN);
			// 	error_id = ERROR_ID_SHANGJIN_COIN_NOT_ENOUGH;
			// 	break;
			// case 9: //惩戒货币
			// 	has_money = (uint32_t)player->get_attr(PLAYER_ATTR_CHENGJIE_COIN);
			// 	error_id = ERROR_ID_CHENGJIE_COIN_NOT_ENOUGH;
			// 	break;
			// case 10: //国御货币
			// 	has_money = (uint32_t)player->get_attr(PLAYER_ATTR_GUOYU_COIN);
			// 	error_id = ERROR_ID_GUOYU_COIN_NOT_ENOUGH;
			// 	break;
		}
		...
		switch(money_type)
		{
			...
			// 	break;
			// case 8: //赏金货币
			// 	player->sub_shangjin_coin(need_money, MAGIC_TYPE_SHOP_BUY);
			// 	break;
			// case 9: //惩戒货币
			// 	player->sub_chengjie_coin(need_money, MAGIC_TYPE_SHOP_BUY);
			// 	break;
			// case 10: //国御货币
			// 	player->sub_guoyu_coin(need_money, MAGIC_TYPE_SHOP_BUY);
			// 	break;
		}
		...
		player->add_achievement_progress(ACType_SHOP_BUY, 0, 0, 1);
	} while(0);
...
static int handle_yuqidao_fill_request(player_struct *player, EXTERN_DATA *extern_data)
{
	...
	do
	{
		...
		mai_info->fill_lv++;
		player->add_achievement_progress(ACType_YUQIDAO_FILL, 0, 0, 1);
		if (mai_info->fill_lv >= (uint32_t)config->GradeNum)
		{
...
static int handle_baguapai_wear_request(player_struct *player, EXTERN_DATA *extern_data)
{
	...
	do
	{
		...
		if (suit_id > 0)
		{
			player->add_achievement_progress(ACType_BAGUA_SUIT, suit_id, 0, 1);
		}
		player->add_achievement_progress(ACType_BAGUA_MIN_STAR, 0, 0, 1);
	} while(0);
...
static int handle_baguapai_decompose_request(player_struct *player, EXTERN_DATA *extern_data)
{
	...
	do
	{
		...
		player->add_achievement_progress(ACType_BAGUA_DECOMPOSE, 0, 0, grid_ids.size());
	} while(0);
...
static int handle_baguapai_refine_star_request(player_struct *player, EXTERN_DATA *extern_data)
{
	...
	do
	{
		...
		player->add_achievement_progress(ACType_BAGUA_REFINE_STAR, 0, 0, 1);
		player->add_achievement_progress(ACType_BAGUA_MIN_STAR, 0, 0, 0);
	} while(0);
...
static int handle_add_chengjie_task_request(player_struct *player, EXTERN_DATA *extern_data)
{
	...
	player->add_achievement_progress(ACType_XUANSHANG, 0, 0, 1);
done:
...
static int handle_sync_guild_skill_request(player_struct *player, EXTERN_DATA *extern_data)
{
	...
	player->add_achievement_progress(ACType_GUILD_SKILL_LEVEL_NUM, 0, 0, 0);
...
static int handle_sync_guild_info_request(player_struct *player, EXTERN_DATA *extern_data)
{
	...
	else if (old_guild_id == 0 && player->data->guild_id > 0)
	{
		...
		player->add_achievement_progress(ACType_GUILD_JOIN, 0, 0, 1);
	}
...
static int handle_guild_answer_award(player_struct *player, EXTERN_DATA *extern_data)
{
	...
	player->add_achievement_progress(ACType_ANSWER, QUESTION_GUILD, 0, 1);
...
static int handle_guild_sync_donation(player_struct * player, EXTERN_DATA * extern_data)
{
	...
	{
		player->add_achievement_progress(ACType_ADD_CURRENCY, ACurrency_GUILD_DONATION, 0, pData->change_val);
	}
...
static int handle_guild_skill_level_up(player_struct * player, EXTERN_DATA * extern_data)
{
	...
	player->add_achievement_progress(ACType_GUILD_SKILL_LEVEL_UP, 0, 0, 1);
...
static int handle_into_zhenying_battle_request(player_struct *player, EXTERN_DATA *extern_data)
{
	...
	player->add_achievement_progress(ACType_ZHENYING_BATTLE, 0, 0, 1);
...
static int handle_get_zhenying_task_award_request(player_struct *player, EXTERN_DATA *extern_data)
{
	...
	if (player->data->zhenying.task_num == table->Num)
	{
		...
		player->add_achievement_progress(ACType_ZHENYING_TASK_AWARD, 0, 0, 1);
	}
...
static int handle_server_chose_zhenying(player_struct *player, EXTERN_DATA *extern_data)
{
	...
	if (req->ret == 0)
	{
		...
		player->add_achievement_progress(ACType_ZHENYING_CHOSE, 0, 0, 1);
	}
...
static int handle_server_change_zhenying(player_struct *player, EXTERN_DATA *extern_data)
{
	...
	if (req->ret == 0)
	{
		...
		player->add_task_progress(TCT_JOIN_CAMP, req->zhenying, 1);
		player->refresh_player_redis_info(false);
		player->add_achievement_progress(ACType_ZHENYING_CHANGE, 0, 0, 1);
	}
...
static int handle_answer_common_question_request(player_struct *player, EXTERN_DATA *extern_data)
{
	...
	if (player->data->common_answer.number == tableNum->parameter1[0]) //完成一次每日答题
	{
		...
		player->add_task_progress(TCT_QUESTION_JOIN, 0, 1);
		player->add_achievement_progress(ACType_ANSWER, QUESTION_DAILY, 0, 1);
		if (player->data->common_answer.right == player->data->common_answer.number)
		{
			player->add_achievement_progress(ACType_DAILY_ANSWER_ALL_RIGHT, 0, 0, 1);
		}
...
static int handle_doufachang_sync_rank(player_struct *player, EXTERN_DATA *extern_data)
{
	...
	player->add_achievement_progress(ACType_DOUFACHANG_RANK, player->data->doufachang_rank, 0, 1);
	if (req->defencer_id > 0)
	{
		...
		if (defencer)
		{
			...
			defencer->add_achievement_progress(ACType_DOUFACHANG_RANK, defencer->data->doufachang_rank, 0, 1);
		}
	}
...
static int handle_doufachang_sync_buy_challenge(player_struct *player, EXTERN_DATA *extern_data)
{
	...
	uint32_t *req = (uint32_t *)get_data();
	uint32_t count = *req;
	player->add_achievement_progress(ACType_DOUFACHANG_BUY_CHALLENGE, 0, 0, count);
...
static int handle_friend_sync_friend_num(player_struct *player, EXTERN_DATA *extern_data)
{
	...
	player->add_achievement_progress(ACType_FRIEND_NUM, 0, 0, friend_num);
	player->add_achievement_progress(ACType_FRIEND_CLOSE_NUM, 0, 0, 0);
...
static int handle_friend_send_gift_success(player_struct *player, EXTERN_DATA *extern_data)
{
	...
	player->add_achievement_progress(ACType_FRIEND_SEND_GIFT, 0, 0, 1);
	return 0;
}

static int handle_friend_is_enemy_answer(player_struct *player, EXTERN_DATA *extern_data)
{
	LOG_INFO("[%s:%d] player[%lu]", __FUNCTION__, __LINE__, extern_data->player_id);
	...
	FRIEND_IS_ENEMY_ANSWER *resp = (FRIEND_IS_ENEMY_ANSWER *)get_data();
	if (resp->is_enemy)
	{
		player->add_achievement_progress(ACType_KILL_ENEMY, 0, 0, 1);
	}
	return 0;
}
...
//		}
		...
		for (uint64_t i = 0; i < config->RecruitNum; ++i)
		{
			...
			if (is_first_time)
			{
				...
				info.type = 2;
				info.data.uuid = 0;
				player->add_partner(config->First, &info.data.uuid);
		...
		player->add_achievement_progress(ACType_PARTNER_RECRUIT, type, 0, 1);
	} while(0);
...
static int handle_partner_bond_active_request(player_struct *player, EXTERN_DATA *extern_data)
{
	...
	do
	{
		...
		{
			player->add_achievement_progress(ACType_PARTNER_BOND_ACTIVE, 0, 0, 1);
		}
...
static int handle_partner_reset_attr_request(player_struct *player, EXTERN_DATA *extern_data)
{
	...
	else
	{
		...
		player->add_achievement_progress(ACType_PARTNER_RESET_ATTR, 0, 0, 1);
	}
...
static int handle_partner_add_attr_request(player_struct *player, EXTERN_DATA *extern_data)
{
	...
	for (uint32_t c = 0; partner->data->attr_cur.base_attr_vaual[i] < partner->data->attr_cur.base_attr_up[i] && c < count; ++c)
	{
		...
		player->add_achievement_progress(ACType_PARTNER_ADD_ATTR, 0, 0, 1);
...
static int handle_partner_add_god_request(player_struct *player, EXTERN_DATA *extern_data)
{
	...
	if (send.ret == 0)
	{
		...
		player->add_achievement_progress(ACType_PARTNER_ADD_GOD, 0, 0, 1);
	}
...
static int handle_partner_fabao_stone_request(player_struct *player, EXTERN_DATA *extern_data)
{
	...
	do{
		...
		player->add_achievement_progress(ACType_FABAO_COMPOSE, get_item_quality(fabao_id), 0, 1);
	}while(0);
...
{	
	...
	do{
		...
		player->add_achievement_progress(ACType_PARTNER_WEAR_FABAO, 0, 0, 1);
	}while(0);
...
//	add_msg_handle(MSG_ID_BAG_SHOW_REQUEST, handle_bag_show_request);
	...
	add_msg_handle(SERVER_PROTO_FRIEND_IS_ENEMY_ANSWER, handle_friend_is_enemy_answer);
