...
//	LOG_DEBUG("%s: %lu cast %u, cd %lu", __FUNCTION__, extern_data->player_id, req->skillid, skill_struct->data->cd_time);
	...
	// 	//攻击的时候, 计算位移
	if (active_config->FlyId != 0 && active_config->CanMove == 2)
	{
...
int handle_team_invite_handle_request(player_struct *player, EXTERN_DATA *extern_data)
{
	...
	if (accept == TEAM_CHOSE__YES)
	{
		if (otherPlayer->m_team == NULL)
		{
			player_struct *tmpArr[MAX_TEAM_MEM] = { player, otherPlayer };
			int ret = 0;
			if (Team::CreateTeam(tmpArr, 1) == NULL)
			{
				ret = 190500002;
			}
			//if (ret != 0)
...
static int handle_qiecuo_start_request(player_struct *player, EXTERN_DATA *extern_data)
{
	...
	player->add_achievement_progress(ACType_QIECUO, 0, 0, 1);
	target->add_achievement_progress(ACType_QIECUO, 0, 0, 1);
...
static int handle_sync_guild_disband(player_struct * /*player*/, EXTERN_DATA * /*extern_data*/)
{
	...
	for (uint32_t i = 0; i < req->player_num; ++i)
	{
		...
		if (member)
		{
			...
			raid_struct *raid = member->get_raid();
			if (raid)
			{
				if (raid->is_guild_battle_raid() || raid->m_config->DengeonRank == DUNGEON_TYPE_GUILD_LAND)
				{
					raid->player_leave_raid(member);
				}
			}
		}
	}
...
//		}
		...
		player->add_achievement_progress(ACType_PARTNER_RECRUIT, 0, 0, 1);
	} while(0);
...
static int handle_partner_fabao_stone_request(player_struct *player, EXTERN_DATA *extern_data)
{
	...
	do{
		...
		player->add_achievement_progress(ACType_FABAO_COMPOSE, fabao_id, 0, 1);
	}while(0);
...
static int handle_fishing_reward_request(player_struct *player, EXTERN_DATA *extern_data)
{
	...
	do
	{
		...
		{
			ret = 190500418;
			LOG_ERR("[%s:%d] player[%lu] bait error, activity_id:%u, bait_id:%u", __FUNCTION__, __LINE__, extern_data->player_id, activity_id, bait_id);
			break;
		}
		...
		player->add_task_progress(TCT_FISHING, bait_id, 1);
	} while(0);
