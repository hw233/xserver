...
int chat_mod::do_gm_cmd(player_struct *player, int argc, char *argv[])
{
	...
	{
		player->send_scene_transfer(0, 100, 100, 100, atoi(argv[2]), 0);
	...
	else if (argc >=1 && strcasecmp(argv[0], "set_script_raid") == 0)
	{
		...
		if (raid && raid->m_config->DengeonRank == DUNGEON_TYPE_SCRIPT)
		{
			...
			for (int i = 0; i < MAX_SCRIPT_COND_NUM; ++i)
				raid->SCRIPT_DATA.script_data.cur_finished_num[i] = 0;
		}
	}
...
void chat_mod::add_bind_gold(player_struct *player, int val)
{
	player->add_bind_gold(val, MAGIC_TYPE_GM);
}

void chat_mod::add_gold(player_struct *player, int val)
{
	player->add_unbind_gold(val, MAGIC_TYPE_GM);
}

void chat_mod::add_prop(player_struct *player, int prop_id, int prop_num)
{
	player->add_item(prop_id, prop_num, MAGIC_TYPE_GM);
}

void chat_mod::add_exp(player_struct *player, int val)
{
	player->add_exp(val, MAGIC_TYPE_GM);
}
...
void chat_mod::gm_add_zhenqi(player_struct *player, int val)
{
	player->add_zhenqi(val, MAGIC_TYPE_GM);
}

void chat_mod::gm_sub_exp(player_struct *player, int val)
{
	player->sub_exp(val, MAGIC_TYPE_GM);
}

void chat_mod::gm_enter_3v3(player_struct *player)
{
	pvp_match_single_ai_player_3(player);	
}

void chat_mod::gm_pvp_ready(player_struct *player)
{
	pvp_match_player_set_ready(player);	
}

void chat_mod::gm_add_guild_popularity(player_struct *player, int val)
{
	player->add_guild_resource(1, val);
}

void chat_mod::gm_add_guild_treasure(player_struct *player, int val)
{
	player->add_guild_resource(2, val);
}

void chat_mod::gm_add_guild_build_board(player_struct *player, int val)
{
	player->add_guild_resource(3, val);
}

void chat_mod::gm_add_guild_donation(player_struct *player, int val)
{
