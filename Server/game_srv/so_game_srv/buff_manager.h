...
//	static int add_buff(buff_struct *p);
	...
	static buff_struct *create_buff(uint64_t id, unit_struct *attack, unit_struct *owner, bool notify = false);
	static int add_skill_buff(unit_struct *attack, unit_struct *owner, int add_num, uint32_t *buff_id);
